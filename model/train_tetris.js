...
function readFile(file) {
  ...
  return new Promise((resolve, reject) => {
    fs.readFile(`./txt/${file}`, "utf8", (err, data) => {
      if (err){
        reject(err);
      } else{ 
        lineReader.eachLine(`./txt/${file}`, function(line) {  
...
const readDir = () => 
    new Promise((resolve, reject) => fs.readdir(`./txt/`, "utf8", (err, data) => err ? reject(err) : resolve(data)));
...
const transformToTensor = (features, labels) => { 
  return tf.tidy(() => {
    ...
    for (let i = 0; i < gestureClasses.length; ++i) {
      const [xTrain, yTrain, xTest, yTest] = convertToTensors(features[i], labels[i], 0.1);
      xTrains.push(xTrain);
      yTrains.push(yTrain);
      xTests.push(xTest);
      yTests.push(yTest);
...
const createModel = async(xTrain, yTrain, xTest, yTest) => {
  const params = {learningRate: 0.001, epochs: 1000};
